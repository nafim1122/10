...
export default function ModelCard({ item, onDeleted }) {
  ...
  const { user } = useAuth()
  const [purchasedCount, setPurchasedCount] = useState(item.purchased || 0)
  const [avgRating, setAvgRating] = useState(item.averageRating || 0)
  const [submittingRating, setSubmittingRating] = useState(false)
  React.useEffect(() => {
    const onModelPurchase = (ev) => {
      const d = ev.detail
      if (d && d.id === item._id) setPurchasedCount(d.purchased)
    }
  ...
  async function doPurchase() {
    ...
    try {
      ...
      const newCount = res.data && (res.data.purchased || (res.data.purchased === 0 ? 0 : undefined))
  ...
  async function submitRating(value){
    if (!user) return toast.show('Please login to rate')
    try{
      setSubmittingRating(true)
      const res = await api.rateModel(item._id, value)
      const avg = res.data && res.data.averageRating
      if (typeof avg === 'number') setAvgRating(avg)
  ...
  <p><strong>Dataset:</strong> {item.dataset}</p>
      ...
      <div style={{marginTop:8}}>
        <small>Purchased: {purchasedCount}</small>
